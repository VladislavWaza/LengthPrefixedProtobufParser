...
TEST(Parser, SlicedData)
{
  ...
  messages = parser.parse(std::string(data->begin(), data->begin()));
  EXPECT_EQ(messages.size(), 0);

  messages = parser.parse(std::string(data->begin(), data->begin() + 1));
  EXPECT_EQ(messages.size(), 0);

  messages = parser.parse(std::string(data->begin() + 1, data->begin() + middle));
  EXPECT_EQ(messages.size(), 0);
